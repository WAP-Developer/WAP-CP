...
{
    public function getAlbum($limit, $start)
    {
        $this->db->order_by('id', 'DESC');
        $album = $this->db->get('wb_album', $limit, $start)->result_array();
        return $album;
    }

    public function getDetailAlbum($id, $limit, $start)
    {
        $query = $this->db->query("SELECT * FROM wb_album a, wb_album_foto b WHERE a.id=b.album_id AND a.slug='$id' ORDER BY b.id_photo DESC LIMIT $limit OFFSET $start")->result_array();
        return $query;
    }
